...
- **负载均衡** - 支持轮询、IP哈希、主备（active-standby）等策略，可配置权重分配
- **故障转移** - 使用系统工具,完成自动故障检测,保持轻量化。支持主备模式（1 主 N 备按顺序接替）
...
严格主备：1 主 N 备按顺序接替。健康检查对所有目标独立探测（不依赖权重），主目标故障时把它的权重置 0、第一个备目标置 1；主+备1都挂时切到备2，以此类推；主恢复后立刻切回主（严格主备，不退化为多活）。底层依然输出 `balance: roundrobin: 0, 1, 0` 给 realm，权重 0 的目标相当于不出现在流量分配里。

使用步骤：先在同一端口下配多个目标（可以是单规则 `REMOTE_HOST` 写逗号分隔的多个地址，或拆成多条规则），然后 `pf` → 负载均衡管理 → 切换负载均衡模式 → 选 `4. 主备模式 (active-standby)`，再 → 开启/关闭故障转移 → 选该端口 → 选 `2. 主备模式`。状态转换与生成的 realm config 示例见 `docs/active-standby-logic.md`。
